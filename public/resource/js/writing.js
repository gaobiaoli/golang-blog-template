...
function uploadImage(file, cb) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      const observable = qiniu.upload(file, "workbench/imgs/" + Date.now() + "_" + file.name, token, putExtra, config)
      const observer = {
        next(res) {
          // ...
        },
        ...
        complete(res) {
          console.log(res)
          cb("http://ryykyck5k.hd-bkt.clouddn.com/" + res.key)
        }
      }
...
$(function () {
  ...
  $(".category").on("click", "li", function (event) {
    ...
    ArticleItem.categoryId = parseInt(target.attr("value"));
    $(".publish-tip").text("");
  });
  // 选择类型
